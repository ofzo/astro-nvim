...
autocmd({"BufAdd", "BufEnter", "TabNewEntered"}, {
    ...
    callback = function(args)
        ...
        if args.buf ~= buf_utils.current_buf then
            buf_utils.last_buf = buf_utils.is_valid(buf_utils.current_buf) and
                                     buf_utils.current_buf or nil
...
autocmd("BufDelete", {
    ...
    callback = function(args)
        ...
        vim.t.bufs = vim.tbl_filter(require("astronvim.utils.buffer").is_valid,
                                    vim.t.bufs)
...
if is_available "alpha-nvim" then
    autocmd({"User", "BufEnter"}, {
        ...
        callback = function(event)
            if ((event.event == "User" and event.file == "AlphaReady") or
                (event.event == "BufEnter" and
                    vim.api.nvim_get_option_value("filetype", {buf = event.buf}) ==
                    "alpha")) and not vim.g.before_alpha then
                vim.g.before_alpha = {
                    showtabline = vim.opt.showtabline:get(),
                    laststatus = vim.opt.laststatus:get()
                }
                vim.opt.showtabline, vim.opt.laststatus = 0, 0
            elseif vim.g.before_alpha and event.event == "BufEnter" and
                vim.api.nvim_get_option_value("buftype", {buf = event.buf}) ~=
                "nofile" then
                vim.opt.laststatus, vim.opt.showtabline = vim.g.before_alpha
                                                              .laststatus, vim.g
                                                              .before_alpha
                                                              .showtabline
    ...
    autocmd("VimEnter", {
        ...
        callback = function()
            ...
            if vim.fn.argc() > 0 or vim.fn.line2byte(vim.fn.line "$") ~= -1 or
                not vim.o.modifiable then
                should_skip = true
            else
        ...
        if vim.g.colors_name then
            for _, module in ipairs {"init", vim.g.colors_name} do
                for group, spec in pairs(
                                       astronvim.user_opts("highlights." ..
                                                               module)) do
...
autocmd({"BufReadPost", "BufNewFile", "BufWritePost"}, {
    ...
    callback = function(args)
        if not (vim.fn.expand "%" == "" or
            vim.api.nvim_get_option_value("buftype", {buf = args.buf}) ==
            "nofile") then
            astroevent "File"
            if require("astronvim.utils.git").file_worktree() or
                utils.cmd({"git", "-C", vim.fn.expand "%:p:h", "rev-parse"},
                          false) then
                astroevent "GitFile"
...
cmd("AstroUpdatePackages",
    function() require("astronvim.utils.updater").update_packages() end,
    {desc = "Update Plugins and Mason"})
cmd("AstroRollback",
    function() require("astronvim.utils.updater").rollback() end,
    {desc = "Rollback AstroNvim"})
cmd("AstroUpdate", function() require("astronvim.utils.updater").update() end,
    {desc = "Update AstroNvim"})
cmd("AstroVersion", function() require("astronvim.utils.updater").version() end,
    {desc = "Check AstroNvim Version"})
cmd("AstroReload", function() require("astronvim.utils").reload() end,
