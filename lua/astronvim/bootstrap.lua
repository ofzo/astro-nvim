...
local function load_module_file(module)
    ...
    for _, config_path in ipairs(astronvim.supported_configs) do
        ...
        local module_path = config_path .. "/lua/" .. module:gsub("%.", "/") ..
                                ".lua"
...
local function func_or_extend(overrides, default, extend)
    ...
    if extend then
        ...
        if type(overrides) == "table" then
            ...
            default = default and vim.tbl_deep_extend("force", default, opts) or
                          opts
...
astronvim.updater = {
    options = astronvim.user_opts("updater",
                                  {remote = "origin", channel = "stable"}),
    snapshot = {
...
astronvim.lsp = {
    skip_setup = astronvim.user_opts("lsp.skip_setup", {}),
    progress = {}
}
--- the default colorscheme to apply on startup
astronvim.default_colorscheme = astronvim.user_opts("colorscheme", "astrotheme",
                                                    false)
