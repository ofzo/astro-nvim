...
function M.numbercolumn(opts)
    ...
    return function()
        ...
        elseif virtnum ~= 0 then
            ...
            local cur = relnum and (rnum > 0 and rnum or (num and lnum or 0)) or
                            lnum
            if opts.thousands and cur > 999 then
                cur = string.reverse(cur):gsub("%d%d%d", "%1" .. opts.thousands)
                          :reverse():gsub("^%" .. opts.thousands, "")
            end
            str =
                (rnum == 0 and not opts.culright and relnum) and cur .. "%=" or
                    "%=" .. cur
        end
    ...
    return function() -- move to M.fold_indicator
        ...
        if width ~= 0 then
            ...
            if foldinfo.level == 0 then
                ...
                for col = 1, width do
                    str = str .. ((vim.v.virtnum ~= 0 and foldsep) or
                              ((closed and
...
function M.showcmd(opts)
    ...
    return status_utils.stylize(("%%%d.%d(%%S%%)"):format(opts.minwid,
                                                          opts.maxwid), opts)
end
...
function M.search_count(opts)
    ...
    return function()
        ...
        if search_ok and type(search) == "table" and search.total then
            return status_utils.stylize(string.format("%s%d/%s%d",
                                                      search.current >
                                                          search.maxcount and
                                                          ">" or "", math.min(
                                                          search.current,
                                                          search.maxcount),
                                                      search.incomplete == 2 and
                                                          ">" or "", math.min(
                                                          search.total,
                                                          search.maxcount)),
                                        opts)
...
function M.ruler(opts)
    ...
    return function()
        ...
        return
            status_utils.stylize(string.format(padding_str, line, char), opts)
    end
end
...
function M.filename(opts)
    ...
    return function(self)
        ...
        return status_utils.stylize((filename == "" and opts.fallback or
                                        filename), opts)
    end
end
...
function M.file_encoding(opts)
    return function(self)
        ...
        return status_utils.stylize(string.upper(
                                        buf_enc ~= "" and buf_enc or vim.o.enc),
                                    opts)
    end
end
...
function M.file_format(opts)
    return function(self)
        ...
        return status_utils.stylize(string.upper(
                                        buf_format ~= "" and buf_format or
                                            vim.o.fileformat), opts)
    end
end
...
function M.unique_path(opts)
    ...
    local function path_parts(bufnr)
        local parts = {}
        for match in (vim.api.nvim_buf_get_name(bufnr) .. "/"):gmatch("(.-)" ..
                                                                          "/") do
    ...
    return function(self)
        ...
        return status_utils.stylize((opts.max_length > 0 and #unique_path >
                                        opts.max_length and
                                        string.sub(unique_path, 1,
                                                   opts.max_length - 2) ..
                                        get_icon "Ellipsis" .. "/") or
                                        unique_path, opts)
    end
end
...
function M.file_modified(opts)
    ...
    return function(self)
        return status_utils.stylize(
                   condition.file_modified((self or {}).bufnr) and opts.str or
                       nil, opts)
    end
end
...
function M.file_read_only(opts)
    ...
    return function(self)
        return status_utils.stylize(
                   condition.file_read_only((self or {}).bufnr) and opts.str or
                       nil, opts)
    end
end
...
function M.git_branch(opts)
    return function(self)
        return status_utils.stylize(vim.b[self and self.bufnr or 0]
                                        .gitsigns_head or "", opts)
    end
end
...
function M.git_diff(opts)
    ...
    return function(self)
        ...
        return status_utils.stylize(status and status[opts.type] and
...
function M.lsp_progress(opts)
    ...
    return function()
        ...
        return status_utils.stylize(Lsp and
                                        (spinner[math.floor(luv.hrtime() / 12e7) %
                                            #spinner + 1] .. table.concat({
                Lsp.title or "", Lsp.message or "",
            ...
            if client.name == "null-ls" and opts.expand_null_ls then
                ...
                for _, type in ipairs {"FORMATTING", "DIAGNOSTICS"} do
                    for _, source in ipairs(
                                         status_utils.null_ls_sources(vim.bo
                                                                          .filetype,
                                                                      type)) do
