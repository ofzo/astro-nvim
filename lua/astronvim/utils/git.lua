...
function git.cmd(args, ...)
    ...
    return require("astronvim.utils").cmd(vim.list_extend({
        "git", "-C", astronvim.install.home
    }, args), ...)
end
...
function git.file_worktree(file, worktrees)
    ...
    for _, worktree in ipairs(worktrees) do
        if require("astronvim.utils").cmd({
            "git", "--work-tree", worktree.toplevel, "--git-dir",
            worktree.gitdir, "ls-files", "--error-unmatch", file
        }, false) then return worktree end
    end
end
...
function git.git_version()
    ...
    if output then
        ...
        local major, min, patch = unpack(
                                      vim.tbl_map(tonumber,
                                                  vim.split(version_str, "%.")))
...
---@return string|nil # The result of the command
function git.hard_reset(dest, ...) return
    git.cmd({"reset", "--hard", dest}, ...) end
...
function git.get_commit_range(start_hash, end_hash, ...)
    local range = start_hash and end_hash and start_hash .. ".." .. end_hash or
                      nil
    local log = git.cmd({
...
function git.parse_remote_url(str)
    return vim.fn.match(str, require("astronvim.utils").url_matcher) == -1 and
               git.url .. str ..
               (vim.fn.match(str, "/") == -1 and "/AstroNvim.git" or ".git") or
               str
end
