...
M.format_opts.filter = function(client)
    ...
    return not (vim.tbl_contains(disabled, client.name) or
               (type(filter) == "function" and not filter(client)))
end
...
M.on_attach = function(client, bufnr)
    ...
    if client.supports_method "textDocument/formatting" and
        not tbl_contains(M.formatting.disabled, client.name) then
        ...
        if autoformat.enabled and
            (tbl_isempty(autoformat.allow_filetypes or {}) or
                tbl_contains(autoformat.allow_filetypes, filetype)) and
            ...
            add_buffer_autocmd("lsp_auto_format", bufnr, {
                ...
                callback = function()
                    ...
                    if autoformat_enabled and
                        ((not autoformat.filter) or autoformat.filter(bufnr)) then
                        vim.lsp.buf.format(
                            extend_tbl(M.format_opts, {bufnr = bufnr}))
                    end
                end
            })
    ...
    if client.supports_method "textDocument/inlayHint" then
        ...
        if vim.lsp.inlay_hint then
            ...
            lsp_mappings.n["<leader>uH"] = {
                function()
                    require("astronvim.utils.ui").toggle_buffer_inlay_hints(
                        bufnr)
                end,
    ...
    if client.supports_method "textDocument/semanticTokens/full" and
        vim.lsp.semantic_tokens then
        ...
        lsp_mappings.n["<leader>uY"] = {
            function()
                require("astronvim.utils.ui").toggle_buffer_semantic_tokens(
                    bufnr)
            end,
...
function M.config(server_name)
    ...
    local lsp_opts = extend_tbl(extend_tbl(
