...
function M.toggle_ui_notifications() -- TODO: rename to toggle_notifications in AstroNvim v4
    ...
    notify(string.format("Notifications %s",
...
function M.toggle_autopairs()
    ...
    if ok then
        ...
        notify(string.format("autopairs %s",
...
function M.toggle_diagnostics()
    ...
    vim.diagnostic.config(require("astronvim.utils.lsp").diagnostics[vim.g
                              .diagnostics_mode])
...
function M.toggle_cmp()
    ...
    notify(ok and string.format("completion %s", bool2str(vim.g.cmp_enabled)) or
...
function M.toggle_autoformat()
    ...
    notify(string.format("Global autoformatting %s",
...
function M.toggle_buffer_autoformat()
    ...
    notify(string.format("Buffer autoformatting %s",
                         bool2str(vim.b.autoformat_enabled)))
end
...
function M.toggle_buffer_semantic_tokens(bufnr)
    ...
    for _, client in ipairs(vim.lsp.get_active_clients()) do
        if client.server_capabilities.semanticTokensProvider then
            vim.lsp.semantic_tokens[vim.b.semantic_tokens_enabled and "start" or
                "stop"](bufnr or 0, client.id)
            notify(string.format("Buffer lsp semantic highlighting %s",
...
function M.toggle_buffer_inlay_hints(bufnr)
    ...
    if vim.lsp.inlay_hint then
        ...
        notify(string.format("Inlay hints %s",
                             bool2str(vim.b.inlay_hints_enabled)))
    end
end
...
function M.set_indent()
    local input_avail, input = pcall(vim.fn.input,
    ...
    if input_avail then
        ...
        notify(string.format("indent=%d %s", indent,
...
function M.change_number()
    ...
    notify(string.format("number %s, relativenumber %s",
...
function M.toggle_foldcolumn()
    ...
    vim.wo.foldcolumn = curr_foldcolumn == "0" and
                            (last_active_foldcolumn or "1") or "0"
