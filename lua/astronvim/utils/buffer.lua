...
function M.is_restorable(bufnr)
    if not M.is_valid(bufnr) or
        vim.api.nvim_get_option_value("bufhidden", {buf = bufnr}) ~= "" then
        return false
    end
    ...
    if vim.tbl_contains(M.sessions.ignore.filetypes, vim.api
                            .nvim_get_option_value("filetype", {buf = bufnr})) or
        vim.tbl_contains(M.sessions.ignore.buftypes, vim.api
                             .nvim_get_option_value("buftype", {buf = bufnr})) then
        return false
    end
    return true
end
...
function M.close(bufnr, force)
    if utils.is_available "mini.bufremove" then
        if not force and
            vim.api.nvim_get_option_value("modified", {buf = bufnr}) then
        ...
        vim.cmd((force and "bd!" or "confirm bd") ..
                    (bufnr == nil and "" or bufnr))
    end
end
...
local function unique_path(bufnr)
    return require("astronvim.utils.status.provider").unique_path() {
        bufnr = bufnr
