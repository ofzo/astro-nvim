    ...
    registry.update(vim.schedule_wrap(function(success, updated_registries)
        if success then
            ...
            if vim.tbl_count(pkg_names) == 0 then
                mason_notify(("Successfully updated %d %s."):format(count,
                                                                    count == 1 and
                                                                        "registry" or
                                                                        "registries"))
            end
            for _, pkg_name in ipairs(pkg_names) do
                ...
                if not pkg_avail then
                    mason_notify(("`%s` is not available"):format(pkg_name),
                        ...
                        pkg:check_new_version(
                            function(update_available, version)
                                if update_available then
                                    mason_notify(
                                        ("Updating `%s` to %s"):format(pkg.name,
                                                                       version.latest_version))
                                    pkg:install():on("closed", function()
                                        mason_notify(
                                            ("Updated %s"):format(pkg.name))
                                    end)
                                else
                                    mason_notify(
                                        ("No updates available for `%s`"):format(
                                            pkg.name))
                                end
                            end)
                    end
            ...
            mason_notify(("Failed to update registries: %s"):format(
                             updated_registries), vim.log.levels.ERROR)
        end
    end))
end
