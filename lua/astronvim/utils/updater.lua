...
function M.generate_snapshot(write)
    ...
    local function git_commit(dir)
        local commit = assert(utils.cmd({"git", "-C", dir, "rev-parse", "HEAD"},
                                        false))
...
function M.version(quiet)
    local version = astronvim.install.version or git.current_version(false) or
                        "unknown"
    if astronvim.updater.options.channel ~= "stable" then
...
function M.create_rollback(write)
    ...
    if write == true then
        ...
        file:write("return " ..
                       vim.inspect(snapshot, {newline = " ", indent = ""}))
        file:close()
    end
...
function M.rollback()
    local rollback_avail, rollback_opts = pcall(dofile,
...
function M.update_available(opts)
    ...
    if not git.is_repo() then
        notify("Updater not available for non-git installations",
               vim.log.levels.ERROR)
        return
    end
    ...
    for remote, entry in pairs(opts.remotes and opts.remotes or {}) do
        ...
        if not current_url then
            ...
            check_needed = true
        elseif current_url ~= url and
            confirm_prompt(("Remote %s is currently: %s\n" ..
                               "Would you like us to set it to %s ?"):format(
                               remote, current_url, url)) then
            git.remote_update(remote, url)
            check_needed = true
        end
        if check_needed and git.remote_url(remote, false) ~= url then
            vim.api.nvim_err_writeln("Error setting up remote " .. remote ..
                                         " to " .. url)
            return
        end
    end
    ...
    if not is_stable then
        local local_branch = (opts.remote == "origin" and "" or
                                 (opts.remote .. "_")) .. opts.branch
        if git.current_branch() ~= local_branch then
            ...
            if not git.checkout(local_branch, false) then
                git.checkout(
                    "-b " .. local_branch .. " " .. opts.remote .. "/" ..
                        opts.branch, false)
            end
        end
        -- check if the branch was switched to successfully
        if git.current_branch() ~= local_branch then
            vim.api.nvim_err_writeln("Error checking out branch: " ..
                                         opts.remote .. "/" .. opts.branch)
            return
        end
    end
    ...
    if is_stable then -- if stable get tag commit
        ...
        if not update.version then -- continue only if stable version is found
            vim.api
                .nvim_err_writeln("Error finding version: " .. version_search)
            return
        end
        update.target = git.tag_commit(update.version)
    elseif opts.commit then -- if commit specified use it
        update.target = git.branch_contains(opts.remote, opts.branch,
                                            opts.commit) and opts.commit or nil
...
function M.update(opts)
    ...
    not opts.skip_prompts and not confirm_prompt(
        ("Update available to %s\nUpdating requires a restart, continue?"):format(
            available_update.version or available_update.target)) then
        ...
        if #breaking > 0 and not opts.skip_prompts and not confirm_prompt(
        ...
        if not updated and not opts.skip_prompts and not confirm_prompt(
        ...
        local summary = {
            ...
            {git.current_version(), "String"}, {"!\n", "Title"}, {
                opts.auto_quit and
                    "AstroNvim will now update plugins and quit.\n\n" or
                    "After plugins update, please restart.\n\n", "WarningMsg"
            }
        }
