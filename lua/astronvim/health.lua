...
function M.check()
    ...
    health.info("AstroNvim Version: " ..
                    require("astronvim.utils.updater").version(true))
    health.info("Neovim Version: v" ..
                    vim.fn
                        .matchstr(vim.fn.execute "version", "NVIM v\\zs[^\n]*"))
    ...
    local programs = {
        {
            ...
            extra_check = function(program)
                ...
                if git_version then
                    if git_version.major < 2 or
                        (git_version.major == 2 and git_version.min < 19) then
                        program.msg =
                            ("Git %s installed, >= 2.19.0 is required"):format(
                                git_version.str)
                    else
                        return true
                    end
            ...
            cmd = {"node"},
            type = "warn",
            msg = "Used for mappings to pull up node REPL (Optional)"
        }, {
            cmd = {"gdu"},
            type = "warn",
    ...
    for _, program in ipairs(programs) do
        ...
        if found then
            ...
            health[program.type](("`%s` is not installed: %s"):format(name,
                                                                      program.msg))
        end
    end
end
